...
class HDBSCANClusterer:
    ...
    def fit(self, X: np.ndarray) -> np.ndarray:
        ...
        logger.info(
            "HDBSCAN: %d clusters, %d noise points (%.1f%%)", n_clusters, n_noise, n_noise / N * 100 if N > 0 else 0
        )
            ...
            if lbl == -1 and score > self.drift_threshold:
                alerts.append(
                    RiskAlert(
                        ...
                        message="Patient trajectory is anomalous — unusual pattern detected",
...
def generate_synthetic_cohort(
    ...
    logger.info(
        "Synthetic cohort: %d patients (avg %.1f visits, %d deteriorations)",
        num_patients,
        num_visits_per_patient,
        inject_deteriorations,
    )
